...
Each Finding must end in one of the following states:
...
An Accepted Finding means:

```text
Finding is valid
        ↓
Risk or gap is acknowledged
        ↓
Required action is accepted
        ↓
Design Spec or implementation requirements must change
```
...
Do not reject a Finding with vague statements such as:
...
```text
Finding:
The Design Spec does not define duplicate operation handling,
recovery behavior, or historical reconciliation.

Decision:
Duplicate operation handling is accepted.
Historical reconciliation is deferred.
The proposed recovery behavior is rejected because the scenario
...
A Duplicate status means:

```text
The Finding was reviewed
        ↓
The issue is already represented elsewhere
        ↓
No independent decision is required
        ↓
The authoritative Finding owns the decision
```
...
until an authorized decision is made.
...
```text
Rejected because this is not necessary.
```
...
The risk is technically possible.
        ↓
However, the affected operation is explicitly limited to one execution
per business day.
        ↓
The consequence is bounded and operationally recoverable.
        ↓
The Finding is rejected for the current scope.
```
...
A valid Deferred decision must answer:

```text
Why is it not being resolved now?
        ↓
When will it be reconsidered?
        ↓
What evidence or event triggers reconsideration?
        ↓
Who is responsible?
```
...
```text
Finding:
The Design Spec does not define:

1. duplicate operation behavior;
2. timeout recovery behavior;
...
Rejected:
The proposed timeout scenario because the operation is synchronous and
the relevant timeout cannot occur under the current system boundary.
```
...
The decision must identify the authoritative Finding:
...
New Evidence
        ↓
Original Premise No Longer Holds
        ↓
Finding Invalidated
```
...
```text
Original Finding:
The external service may return duplicate records.

New Evidence:
The confirmed API contract guarantees a globally unique record identifier
and the service rejects duplicate creation requests.
...
The decision record must be traceable through:
...
│
└── Required Action
    └── Define duplicate-operation behavior
```
...
```text
CR-004:
The Design Spec should reject duplicate operations.

CR-007:
The Design Spec must allow duplicate operations for separate business events.
```
...
If the conflict cannot be resolved, the relevant Finding status should remain:

```text
PENDING_DECISION
```

until an authorized decision-maker resolves the conflict.
