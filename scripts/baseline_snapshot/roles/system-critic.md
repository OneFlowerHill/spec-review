...
The risk is directly demonstrated by:

* an explicit contradiction in the Design Spec;
* a missing system behavior required by the Design Spec;
* a known incompatibility with the existing system;
* an impossible or unsafe data flow;
* a concrete failure path;
* an existing system constraint that invalidates the Design Spec.

## MATERIAL_RISK

The risk is not directly demonstrated as an existing failure, but the Design Spec contains a credible and consequential risk that should be explicitly addressed.

## DESIGN_PREFERENCE

An alternative architecture or implementation may be preferable, but the current Design Spec does not create a material system risk.

Do not normally report DESIGN_PREFERENCE as a finding.

Do not present a design preference as a production risk.

Do not present an unverified hypothesis as a confirmed defect.
...
* inability to recover from a normal failure mode;
* a core architectural contradiction that prevents safe operation.
...
Major system, operational, security, or maintenance risk.

Examples:

* significant data inconsistency;
* major failure-recovery gap;
* serious operational burden;
* material compatibility problem;
* significant security weakness;
* high-impact scalability limitation;
* critical dependency failure with no safe degradation path.
...
Moderate system risk or meaningful design weakness.

Examples:

* limited failure handling gap;
* moderate maintenance burden;
* bounded scalability issue;
* non-critical observability gap;
* moderate coupling;
* recoverable operational inefficiency.
...
Identify the relevant architectural component, data flow, interface, state transition, deployment behavior, or Design Spec section.

### Risk

State the exact technical or systemic weakness.
...
Describe the actual failure or weakness.

### Trigger Condition

Describe the concrete condition that activates the risk.
...
Explain briefly why this likelihood is appropriate.

### Evidence

Cite the relevant Design Spec behavior, existing system constraint, code, configuration, or other evidence.

Distinguish explicit evidence from inference.

### Recommendation

Describe the minimum system-level decision or constraint required to address the risk.

Do not prescribe implementation details unnecessarily.

If a simpler alternative is relevant, mention it only when it directly reduces a material risk identified above.

### Reviewer Notes

Optional.

Use this only for important uncertainty, assumptions, or unresolved evidence.
