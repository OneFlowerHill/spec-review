...
Every Finding must use exactly one Evidence Class.
...
Do not inflate P2 issues to higher severities (see §15).

---

## 5. Confidence

置信度 HIGH/MEDIUM/LOW 的三档语义见 `references/common.md` §3.1；本协议的置信度使用规则如下：置信度须与证据强度匹配（见 §16）。
...
Do not use confidence to reduce severity automatically. A low-confidence P0 risk may still require explicit investigation.
